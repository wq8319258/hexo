...
void win_diamond::withdraw(const account_name account, asset quantity) {
    ...
    action(
            ...
            std::make_tuple(_this_contract, account, quantity, std::string("zcj.fomo.io"))
...
void win_diamond::on(const currency::transfer &t, account_name code) {
    ...
    // pay dividend
    const uint64_t dividend =  _players.begin() == _players.end() ? 0 : (amount * DIVIDEND_SIZE);
